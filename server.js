...
// ── Auto-detect 3uTools install path ──────────────────────────────────────────
function detect3uToolsPath(sub) {
  const candidates = [
    path.join("C:\\", "3uTools9", sub),
    path.join("C:\\", "3uTools", sub),
    path.join(HOME, "3uTools9", sub),
    path.join(HOME, "3uTools", sub),
    path.join("C:\\Users\\Public", "3uTools9", sub),
    path.join("C:\\Users\\Public", "3uTools", sub),
  ];
  // Thêm tất cả thư mục bắt đầu bằng "3uTools" ở gốc C:\
  try {
    const roots = fs.readdirSync("C:\\").filter(n => /^3utools/i.test(n));
    for (const r of roots) candidates.unshift(path.join("C:\\", r, sub));
  } catch {}
  const found = candidates.find(p => { try { return fs.existsSync(p); } catch { return false; } });
  return found || candidates[0]; // fallback về candidates[0] nếu chưa cài
}
...
const CATALOG_WIN = [
  ...
  // ── 3uTOOLS ──
  {
    ...
    tip: `Đường dẫn đã phát hiện: ${_3U_FIRMWARE}. Kiểm tra ngay — dễ tiết kiệm 10–50 GB.`,
  },
  {
...
function deleteItem(itemPath) {
  return new Promise((resolve) => {
    if (IS_WIN) {
      ...
      // Xóa nội dung bên trong (giữ lại folder cha) — an toàn hơn cho system/cache dirs
      const ps = `Get-ChildItem -Path '${escaped}' -Force -ErrorAction SilentlyContinue | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue; exit 0`;
      exec(`powershell -NoProfile -Command "${ps}"`, (_err, _out, stderr) => {
        if (stderr && /Access.*denied|UnauthorizedAccess/i.test(stderr)) {
          resolve(new Error("Cần quyền Administrator để xóa thư mục hệ thống này. Hãy chạy lại app bằng 'Run as administrator'"));
        } else {
          resolve(null); // partial delete chấp nhận được (một số file đang bị lock)
        }
      });
    } else {
...
app.post("/api/operation", async (req, res) => {
  ...
  if (type === "delete" || type === "recycle") {
    ...
    if (type === "recycle" && IS_WIN) {
      ...
      // — Windows Update Cache: phải dừng service trước mới xóa được —
      if (IS_WIN && id === "win-update-cache") {
        sseWrite(res, { type: "info", message: "⏸️  Dừng Windows Update service (wuauserv)..." });
        await new Promise(r => exec("net stop wuauserv", r));
      ...
      if (IS_WIN && id === "win-update-cache") {
        await new Promise(r => exec("net start wuauserv", r));
        sseWrite(res, { type: "info", message: "▶️  Windows Update service đã khởi động lại" });
      }
  ...
  return new Promise(r => {
    ...
    exec(`powershell -NoProfile -Command "${ps}"`, (e, o) => {
      try {
        ...
        // Fallback: dùng netstat cho Windows cũ không có Get-NetTCPConnection
        exec(`netstat -ano`, (_e2, o2) => {
          const seen = new Set(), ports = [];
          (o2||"").split("\n").filter(Boolean).forEach(line => {
            const m = line.match(/TCP\s+([\d.:\[\]]+):(\d+)\s+\S+\s+LISTENING\s+(\d+)/i);
            if (!m) return;
            const addr = (m[1]==="0.0.0.0"||m[1]==="[::]"||m[1]==="::")?"0.0.0.0":m[1];
            const port = parseInt(m[2]), pid = parseInt(m[3]);
            const k = `TCP:${port}`; if (seen.has(k)) return; seen.add(k);
            if (!isNaN(port) && port > 0) ports.push(enrichPort(port, pid, "?", "TCP", addr));
          });
          r(ports.sort((a,b) => a.port - b.port));
        });
      }
...
app.post("/api/license/activate", (req, res) => {
  ...
  if (!key || typeof key !== "string" || key.trim().length < 10) {
    return res.status(400).json({ ok: false, message: "Vui lòng nhập key hợp lệ." });
  }
...
app.use("/api", (req, res, next) => {
  ...
  if (!license.isAllowed()) {
    return res.status(402).json({
      ...
      message: "Hết hạn sử dụng. Vui lòng nhập key để tiếp tục.",
    });
  }
  next();
});
...
app.listen(PORT, () => {
  ...
  const licLine = lic.status === "activated"
    ? `  │   ✅ License: ${lic.licenseType} | Hết hạn: ${new Date(lic.expiryDate).toLocaleDateString("vi-VN")}`
    : lic.status === "trial"
    ? `  │   ⏳ Dùng thử: còn ${lic.daysLeft} ngày`
    : `  │   ⚠️  Hết hạn! Nhập key tại ${url}`;

  console.log(`\n  ╔══════════════════════════════════════════╗`);
  console.log(`  ║   🖥️  Storage Optimizer — ${PLATFORM_INFO.label.padEnd(14)}║`);
  console.log(`  ╚══════════════════════════════════════════╝`);
  console.log(licLine);
  console.log(`\n  ➜  ${url}\n`);
